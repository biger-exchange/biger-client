package biger.client.ws.react.websocket;
...
public class BigerWebSocketClient<T> {
    ...
    private final Function<String, T> text2MessageConvertorFunc;

    private final Function<T, String> msgIdExtractorFunc;
    ...
    private final Map<String, Flux<T>> topic2FluxMap = new HashMap<>();
    ...
    public BigerWebSocketClient(String address, Function<String, T> text2MsgFunc, Function<T, String> msg2SubIdFunc) {
        this(address, text2MsgFunc, msg2SubIdFunc, DEFAULT_MAX_FRAME_SIZE, DEFAULT_CONNECT_TIMEOUT, DEFAULT_RETRY_TIMEOUT);
    }

    public BigerWebSocketClient(String address, Function<String, T> text2MsgFunc, Function<T, String> msg2SubIdFunc, int maxFramePayload, Duration connectionTimeout, Duration retryTimeout) {
        this.wsUri = URI.create(address);
        ...
        this.unkonwMessageFlux = Flux.<T>create(emitter -> {
    ...
    private void onMessageReceived(String text) {
        ...
        T msg = Optional.ofNullable(text).map(this.text2MessageConvertorFunc).orElse(null);
        if (msg == null) {
    ...
    private void dispatchMessage(String subId, T message) {
    ...
    public Mono<T> expectAck(String requsetId) {
    ...
    public Mono<T> requestSingle(String requestId, String requestMsg) {
    ...
    public void addAckMonoSink(String requestId, MonoSink<T> monoSink) {
    ...
    public Flux<T> sub(String subId, String subRequestMsg, String unSubRequestMsg) {
        return this.sub(subId, subRequestMsg, unSubRequestMsg, FluxSink.OverflowStrategy.LATEST);
    }

    public Flux<T> sub(String subId, String subRequestMsg, String unSubRequestMsg, FluxSink.OverflowStrategy overflowStrategy) {
        LOG.debug("Subscribing to websocket Channel {}", subId);
        Lock lock = locks.computeIfAbsent(subId, k-> new ReentrantLock());
        lock.lock();
        try {
            Flux<T> sharedFlux = this.topic2FluxMap.get(subId);
            if (sharedFlux != null) {
                return sharedFlux.share();
            }

            Flux<T> ans = Flux.<T>create(fluxSink -> {
    ...
    public Flux<T> sub(String subId, BiFunction<String, Object[], String> requestMsgFunc, Object[] args, BiFunction<String, Object[], String> unSubRequestMsgFunc, FluxSink.OverflowStrategy overflowStrategy) {
